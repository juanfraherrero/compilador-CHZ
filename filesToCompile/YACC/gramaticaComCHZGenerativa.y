...
ejecutable  :    asignacion
            |    invocacion                                 
            |    seleccion
            |    PRINT CADENA_CARACTERES                    { Tercet t = Tercet("print", $2, ""); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
            |    ciclo_while
            |    acceso_objeto
            |    PRINT                                      { yyerror("Se detectó la falta de una cadena de caracteres al querer imprimir");}
            ;

asignacion : IDENTIFICADOR '=' expresion_aritmetica          { Tercet t = Tercet("=", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
           ;
...
expresion_aritmetica : expresion_aritmetica '+' termino         { Tercet t = Tercet("+", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | expresion_aritmetica '-' termino          { Tercet t = Tercet("-", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | expresion_aritmetica '-' '*' termino      { yywarning("Se detectó un error en operador, quedará '-'"); Tercet t = Tercet("-", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | expresion_aritmetica '+' '*' termino      { yywarning("Se detectó un error en operador, quedará '+'"); Tercet t = Tercet("+", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | expresion_aritmetica '-' '/' termino      { yywarning("Se detectó un error en operador, quedará '-'"); Tercet t = Tercet("-", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | expresion_aritmetica '+' '/' termino      { yywarning("Se detectó un error en operador, quedará '+'"); Tercet t = Tercet("+", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
                    | termino                                   { $$ = $1; }
...
termino : termino '*' factor                                    { Tercet t = Tercet("*", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
        | termino '/' factor                                    { Tercet t = Tercet("/", $1, $3); int number = tercets.add(t); $$ = charTercetoId + to_string(number); }
        | factor                                                { $$ = $1; }
        ;

seleccion : IF bloque_condicion cuerpo_if                       { Tercet *t = tercets.pop(); t->setSecondArg( charTercetoId + to_string(tercets.numberOfLastTercet() + 1) ); yyPrintInLine("Se detectó un bloque de control IF-ELSE");}                     
...
bloque_condicion : '(' condicion ')'                            { Tercet * t = Tercet("BF", charTercetoId + to_string(tercets.numberOfLastTercet()), ""); int number = tercets.add(t); tercets.push(t); $$ = charTercetoId + to_string(number); }
                 ;
...
cuerpo_if : cuerpo_then ELSE cuerpo_else END_IF
          ...
          | cuerpo_then END_IF                                  { yyPrintInLine("Se detectó un bloque de control IF");}
          ; 
